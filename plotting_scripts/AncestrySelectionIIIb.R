...
###################################################################
lctsnp <- 16104
copiedbyrecipient <- t(h5read(datafile,paste0("paintings/chrom02/nonlocal"),index = list(tmp,lctsnp)))

# 2 haps per sample!!
tmp2 <- c()
for(i in hap2sampleindex(copiedbyrecipient,10)) tmp2 <- c(tmp2,i:(i+9))
copiedbycopier <- t(h5read(datafile,paste0("paintings/chrom02/local"),index = list(tmp2,lctsnp)))
newprops <- matrix(0,nc=length(regions),nr=n_haps*n_samps)
colnames(newprops)<- regions
cnt <- 1
for(i in seq(1,length(copiedbycopier),by= 10))
{
  for(j in regions)
  {
    newprops[cnt,j] <- sum(hapregs[copiedbycopier[1,(i:(i+9))]]==j)
  }
  cnt <- cnt+1
  print(cnt)
}

newprops <- newprops/n_samps
copiedbycopiersamps <- cbind(1:(n_haps*10),hapregs[copiedbyrecipient],apply(newprops,1,which.max))
colnames(copiedbycopiersamps) <- c("hap","reg_copied","maxreg_copiedbycopier")

## WEHICH COPY FROM EURO
copyeuro <- which(copiedbycopiersamps[,1]==8)
euro <- which(copiedbycopiersamps[,1]==8&copiedbycopiersamps[,2]==8)
noteuro <- which(copiedbycopiersamps[,1]==8&copiedbycopiersamps[,2]!=8)

## NOW LOOK OVER TEN SAMPLES PER HAP
newprops2 <- matrix(0,nr=n_haps,nc=4)
cnt <- 1
for(i in seq(1,nrow(copiedbycopiersamps),by=10))
{
  perhap <- copiedbycopiersamps[i:(i+9),]
  newprops2[cnt,1] <- as.numeric(names(which.max(table(perhap[,2]))))
  newprops2[cnt,2] <- as.numeric(names(which.max(table(perhap[,3]))))
  newprops2[cnt,3] <- newprops2[cnt,1] == newprops2[cnt,2]
  newprops2[cnt,4] <- (newprops2[cnt,1] == newprops2[cnt,2]) & newprops2[cnt,1] == 8
  cnt <- cnt+1
}
