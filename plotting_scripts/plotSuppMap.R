...
setwd("~/repos/admixture_in_africa/")
...
source(paste0(main_dir,"admixture_in_africa/packages_dev/functionWriter.R"))
setwd(paste0(main_dir,"admixture_in_africa/"))
###########################################################
## DEFINE DATAFILES
...
regions <- ancreg_list <- c("Western_Africa_Niger-Congo","Central_West_Africa_Niger-Congo",
...
legend_text <- gsub("_"," ",ancreg_list)
legend_text <- gsub("Eurasia","Eurasia  (not shown)",legend_text)
legend("bottomleft", legend = legend_text,
       fill = pcolshex, border = pcolshex, title = "ANCESTRY REGIONS", bty = "n",
       cex = 1.2)
